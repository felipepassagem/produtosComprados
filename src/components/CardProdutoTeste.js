...
import { Container, Card, Table, Button, Col, Row, Form, Carousel, OverlayTrigger, Tooltip, ListGroupItem, ListGroup } from "react-bootstrap";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faCoffee, faStepBackward } from '@fortawesome/free-solid-svg-icons'
...
function CardProdutoTeste({ data }) {
    const { id } = useParams();
    ...
    /*  useEffect(() => {
         for (var item in dataTest2) {
             console.log()
             if (item['obj'] == id) {
                 console.log(item['0bj'])
             }
         }
     }, [produto]) */

    /* console.log(dataTest2['obj']['idproduto']) */

    /* console.log(item)
    setProduto(dataTest2[item]) */
    const handleSlide = () => {
        var upList = []
        var downList = []
        ...
        /* if(nFotos == 1){
            for (var i in btnList[0]){
                btnList[0][i].style.visibility = "hidden";
            } 
        } else {
            for (var i in btnList[0]){
                console.log(btnList[0][i])
                btnList[0][i].style.visibility = "visible";
            } 
        } */
            ...
            {produto != undefined ?
                <Container fluid>
                    ...
                    <Row className="justify-content-center pt-5" >
                        ...
                        <Col className='d-flex justify-content-center embed-responsive' md={6}>

                            <Card style={{ width: '18rem' }}>
                                <Card.Body>
                                <Card.Title>Card Title</Card.Title>
                                    <Carousel className='carousel embed-responsive-item' interval={null} activeIndex={index} onSelect={handleSelect} /* fade={true} */ indicators={false} /* onSlid={(e) => { handleSlide() }} */ onSlide={() => { handleSlide() }}>


                                        {fotos.map((foto, i) => {
                                            return (
                                                <Carousel.Item key={i} className="car-img" style={{ width: "25rem", height: "30rem", textAlign: 'center', /* blockSize: 'fit-content' */ }}
                                                    id={i}
                                                >
                                                    {foto['fotos'].map((f, ind) => {
                                                        ...
                                                        return (
                                                            <div /* style={{ position: 'relative' }} */>
                                                                ...
                                                                <img style={{ width: "100%", height: "100%", minHeight: '30rem' }}
                                ...
                                <Card.Subtitle  className=" text-muted pt-2">Ref.: {dataTest['referencia']}</Card.Subtitle>
                                    <Card.Subtitle  className="mb-3 text-muted pt-2 fw-bold">Cor: {dataTest['grades'][index]['cor'].split(";")[2]}</Card.Subtitle>
                                    <Card.Subtitle  className="mb-2 card-subtitle card-subtitle">Descrição</Card.Subtitle>
