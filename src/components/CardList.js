        ...
        <Fragment>
            ...
            >
                <div className='card-img-div' style={
                    ...
                    {backgroundImage: "url(" + `https://server.sistemaagely.com.br/${data['link']}` + ")", borderRadius: '10px' ,backgroundSize: 'cover',backgroundPositionX: 'center', backgroundPositionY: 'center', height: '20rem'} :
                    value == 1 && screenWidth < screenHeight && parseFloat(screenHeight / screenWidth) > 1.6 ?
                    {backgroundImage: "url(" + `https://server.sistemaagely.com.br/${data['link']}` + ")", borderRadius: '10px' , backgroundPositionX: 'center', backgroundPositionY: 'center',backgroundSize: 'cover', height: '11rem'} :
                    value == 1 && parseFloat(screenHeight / screenWidth) < 1.6 && screenWidth < screenHeight  ?
                    {backgroundImage: "url(" + `https://server.sistemaagely.com.br/${data['link']}` + ")", borderRadius: '10px' , backgroundPositionX: 'center', backgroundPositionY: 'center',backgroundSize: 'cover', height: '18rem'} :
                    value == 2 && screenWidth < screenHeight && parseFloat(screenHeight / screenWidth) < 1.6 ? 
                    {backgroundImage: "url(" + `https://server.sistemaagely.com.br/${data['link']}` + ")", borderRadius: '10px'  , backgroundPositionX: 'center', backgroundPositionY: 'center',backgroundSize: 'cover', height: '16rem'} :
                    value == 2 && screenWidth < screenHeight && parseFloat(screenHeight / screenWidth) > 1.6 ? 
                ...
                <Card.Body className="p-1">
                    ...
                    <Card.Text className={screenHeight > screenWidth && value == 2 ? "p-0 m-0 text-muted grid3-font" : screenHeight > screenWidth && value == 1 ? "p-0 m-0 text-muted fs-7" : "p-0 m-0 text-muted fs-7"}>R$ {parseFloat(data['valormax']).toLocaleString('pt-br', {minimumFractionDigits: 2})}</Card.Text>
                </Card.Body>
            </Card>
        </Fragment>
    );
